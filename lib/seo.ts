...
export const baseKeywords = {
  id: [
    'gaji PNS', 'tunjangan kinerja', 'transparansi pemerintah', 'pegawai negeri sipil',
    'gaji pejabat publik', 'kompensasi PNS', 'golongan PNS', 'eselon',
    'kalkulator gaji', 'data gaji pemerintah', 'forpublic.id'
  ],
  en: [
    'Indonesian civil service salary', 'PNS salary', 'government transparency', 
    'public officials salary', 'civil servant compensation', 'performance allowance',
    'salary calculator', 'government accountability', 'Indonesian government salary'
  ]
}

// Generate page-specific keywords
export function generatePageKeywords(page: string, locale: string, additionalKeywords: string[] = []): string[] {
...
function getPageSpecificKeywords(page: string, locale: string): string[] {
  if (locale === 'id') {
    switch (page) {
      case 'calculator':
        return ['kalkulator gaji PNS', 'hitung gaji pegawai negeri sipil', 'estimasi gaji PNS', 'simulator gaji']
      case 'browse':
        return ['data gaji PNS', 'database gaji pemerintah', 'perbandingan gaji PNS', 'struktur gaji kementerian']
      case 'analysis':
        return ['analisis gaji PNS', 'statistik gaji pemerintah', 'tren gaji PNS', 'visualisasi data gaji']
      case 'officials':
        return ['gaji pejabat negara', 'gaji presiden', 'gaji menteri', 'gaji gubernur', 'gaji DPR']
      default:
        return []
    }
  } else {
    switch (page) {
      case 'calculator':
        return ['civil service salary calculator', 'PNS salary estimator', 'government salary calculator']
      case 'browse':
        return ['civil service salary data', 'government salary database', 'ministry salary comparison']
      case 'analysis':
        return ['salary analysis', 'government salary statistics', 'compensation trends']
      case 'officials':
        return ['government officials salary', 'president salary', 'minister compensation']
      default:
        return []
    }
  }
...
export function generateBrowseDescription(dataCount: number, locale: string): string {
  if (locale === 'id') {
...
export function generateOfficialsDescription(officialsCount: number, locale: string): string {
  if (locale === 'id') {
...
export function generateStructuredData(type: 'website' | 'organization' | 'dataset' | 'government', data: any) {
  ...
  switch (type) {
    case 'website':
      return {
        ...
        potentialAction: {
          '@type': 'SearchAction',
          target: 'https://salary.forpublic.id/id/browse?q={search_term_string}',
    ...
    case 'government':
      return {
        ...
        description: 'Official salary information for Indonesian civil servants and public officials',
        provider: {
