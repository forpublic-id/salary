...
export function SalaryCharts({ golonganData, tunjanganData, locale }: SalaryChartsProps) {
  ...
  const salaryDistribution = useMemo(() => {
    ...
    return ranges.map(range => ({
      range: range.range,
      count: golonganData.filter(g => 
        g.gajiPokok >= range.min && g.gajiPokok < range.max
      ).length
    }))
  }, [golonganData])
  ...
  const ministryComparison = useMemo(() => {
    ...
    return Object.entries(ministryAverage)
      .map(([ministry, data]) => ({
        ministry: ministry.length > 25 ? ministry.slice(0, 25) + '...' : ministry,
        fullName: ministry,
  ...
  const allowanceByCategory = useMemo(() => {
    ...
    tunjanganData.forEach(tunjangan => {
      categories[tunjangan.kategori] = (categories[tunjangan.kategori] || 0) + tunjangan.nominal
    })
      ...
      <Card>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={400}>
            <BarChart data={salaryByGradeData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  ...
                  cy="50%"
                  labelLine={false}
                  label={({ category, total }) => `${category}: ${(total / 1000000000).toFixed(1)}B`}
                  outerRadius={80}
                  fill="#8884d8"
                  dataKey="total"
                >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="text-2xl font-bold">
              {formatCurrency(
                Math.round(golonganData.reduce((sum, g) => sum + g.gajiPokok, 0) / golonganData.length),
                locale
              )}
            </div>
            <p className="text-xs text-muted-foreground">
